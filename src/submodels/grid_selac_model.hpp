...
TOKEN(g_alloc_suffstat)

struct selac {

    template <class Gen>
    static auto make(PreparedData& data, size_t ncat, double xmin, double xmax, Gen& gen) {
        ...
        raw_value(wcom) = grantham_wcom_selac;
        ...
        // expression level
        auto psi = make_node<normal>(one_to_const(0.0), one_to_const(1.0));
        draw(psi, gen);
        get<value>(psi) = 0;
        ...
        auto profile_array = make_dnode_array_with_init<selac_profilearray>(
                ...
                [&ncat, &xmin, &xmax] (int cat) {
                    return exp(xmin + (xmax-xmin)*cat/ncat);
                });
        ...
        auto g_var = make_node<gamma_mi>(one_to_const(1.0), one_to_const(1.0));
        draw(g_var, gen);

        auto g_weights = make_dnode_with_init<custom_dnode<std::vector<double>>>(
                std::vector<double>(ncat,1.0),
                [ncat, xmin, xmax, &v=get<value>(g_var), &m=get<value>(psi)] 
                () {
                    return 
                    std::function<void(std::vector<double>&)>(
                        [ncat, xmin, xmax, &m, &v] (std::vector<double>& y)   {
                            assert(y.size() == ncat);
                            double tot = 0;
                            for (size_t cat=0; cat<ncat; cat++)    {
                                double x = xmin + (xmax-xmin)*cat/ncat;
                                y[cat] = exp(-(x-m)*(x-m)/v);
                                tot += y[cat];
                            }
                            for (size_t cat=0; cat<ncat; cat++)    {
        ...
        auto g_alloc = make_node_array<categorical>(nsite, n_to_one(g_weights));
        draw(g_alloc, gen);
        ...
        // alloction suff stat (= current occupancies of mixture components)
        // useful for resampling g_weigths
        auto g_alloc_ss = mixss_factory::make_alloc_suffstat(ncat, get<value>(g_alloc));
        ...
        return make_model(
            ...
            g_alloc_suffstat_ = move(g_alloc_ss),
    ...
    template<class Model, class Gen>
        static auto move_hyper_selac(Model& model, Gen& gen) {

            g_alloc_suffstat_(model).gather();
            auto logprob = suffstat_logprob(g_weights_(model), g_alloc_suffstat_(model));
            auto update = simple_gather(g_weights_(model));

            sliding_move(psi_(model), logprob, 1, 10, gen, update);
            sliding_move(psi_(model), logprob, 0.3, 10, gen, update);
            scaling_move(g_var_(model), logprob, 1, 10, gen, update);
            scaling_move(g_var_(model), logprob, 0.3, 10, gen, update);
        }
