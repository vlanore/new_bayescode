...
struct globom_slave {
    ...
    static auto make(PreparedData& data, Gen& gen) {
        ...
        //     [& mat = get<value>(codon_submatrix), &pss = *path_suffstats](
        //         auto& omss) { omss.AddSuffStat(mat, pss.get()); });

        return make_model(global_omega_ = move(global_omega),
            branch_lengths_ = move(branch_lengths), nuc_rates_ = move(nuc_rates),
            // codon_statespace_ = codon_statespace,
            codon_submatrix_ = move(codon_submatrix), phyloprocess_ = move(phyloprocess),
            bl_suffstats_ = move(bl_suffstats), path_suffstats_ = move(path_suffstats),
            nucpath_suffstats_ = move(nucpath_ss)//, omegapath_suffstats_ = move(omega_ss)
            );
    }
