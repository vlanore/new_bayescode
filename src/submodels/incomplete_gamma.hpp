...
//-----------------------------------------------------------------------------
double PointChi2(double prob, double v) {
    double e = 0.5e-6, aa = 0.6931471805, p = prob, g, xx, c, ch, a = 0.0, q = 0.0, p1 = 0.0,
           p2 = 0.0, t = 0.0, x = 0.0, b = 0.0, s1, s2, s3, s4, s5, s6;

    if (p < 0.000002 || p > 0.999998 || v <= 0.0) return (-1.0);
    g = LnGamma(v / 2.0);
    xx = v / 2.0;
    c = xx - 1.0;
...
l4:
    ...
    ch +=
        t * (1 + 0.5 * t * s1 - b * c * (s1 - b * (s2 - b * (s3 - b * (s4 - b * (s5 - b * s6))))));
    if (fabs(q / ch - 1.0) > e) goto l4;
    return (ch);
}
...

{
    double a0 = -0.322232431088, a1 = -1.0, a2 = -0.342242088547, a3 = -0.0204231210245,
           a4 = -0.453642210148e-4, b0 = 0.0993484626060, b1 = 0.588581570495, b2 = 0.531103462366,
           b3 = 0.103537752850, b4 = 0.0038560700634, y, z = 0, p = prob, p1;
...
double Chi2Pval(double x, int df) {
    double p = 0;

    x /= 2;
    if ((df == 1) || (df == 2)) {
        double alpha = ((double)df) / 2;
        double l = Random::logGamma(alpha);
        p = 1 - IncompleteGamma(x, alpha, l);
    } else if (df == -1) {
        double alpha = 0.5;
        double l = Random::logGamma(alpha);
        p = 0.5 * (1 - IncompleteGamma(x, alpha, l));
    } else if (df == -2) {
        double alpha1 = 0.5;
        double alpha2 = 1.0;
        double l1 = Random::logGamma(alpha1);
        double l2 = Random::logGamma(alpha2);
        double p1 = 1 - IncompleteGamma(x, alpha1, l1);
        double p2 = 1 - IncompleteGamma(x, alpha2, l2);
        p = 0.25 * p2 + 0.5 * p1;
    }
    return p;
}
